...
('shaun_paid_content.enable', 'Enable', 'If disabled, content creators cannot paid content, members cannot subscribe to content creator’s profile, and all paid posts will be hidden.', '0', '', 'checkbox', 1, 2, (SELECT `id` FROM `{prefix}setting_group_subs` WHERE `key` = 'paid_content'), 0),
